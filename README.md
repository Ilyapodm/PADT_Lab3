...
Оператор "(i, j)" реализован в самом интерфейсе и доступен всем потомкам при переопределении get(i, j).

#### Детали реализации

iabs() - единый интерфейс (через перегрузку) вычисления модуля для int, double, T и Complex\<T\>
