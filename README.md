...
Метод `in_bounds()` не был вынесен в `IMatrix`, так как он сложный (всего пару строк, можно и написать несколько раз), а при его использовании в таком случае дергались бы 2 виртуальных метода - overhead.
