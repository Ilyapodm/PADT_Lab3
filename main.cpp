...
static int read_int(const std::string& prompt) {
    ...
    for (;;) {
        ...
        if (std::cin.eof()) throw std::runtime_error("stdin closed");
...
// output the matrix with the type using dynamic_cast
...
static void fill_manually(IMatrix<T>* m) {
    ...
    for (int i = 0; i < m->get_rows(); i++)
        for (int j = 0; j < m->get_cols(); j++) {
            double v = read_double(" [" + std::to_string(i) + "]["
                                        + std::to_string(j) + "] = ");
            try { m->set(i, j, v); }
            catch (const std::invalid_argument&) {
                std::cout << " [!] Position (" << i << "," << j
                          << ") outside structure, skipped.\n";
            }
        }
}
...
// for SparseMatrix: fill only density*rows*cols cells
// fill_randomly creates dense matrix, so lose the sense of sparse
static void fill_randomly_sparse(SparseMatrix<T>* m, unsigned seed, double density) {
    const int rows = m->get_rows();
    const int cols = m->get_cols();
    const int total = rows * cols;
    const int nnz   = static_cast<int>(density * total);
    ...
    while (inserted < nnz) {
        int i = row_dist(rng);
        int j = col_dist(rng);
        // set will rewrite if the cell already exists
        double v = AlgebraTraits<T>::random_value(rng, val_dist);
        if (v != 0.0) {
            m->set(i, j, v);
            ++inserted;
        }
...
static IMatrix<T>* create_matrix() {
    ...
    if (type == 6) {
        ...
        SparseMatrix<T>* sm = new SparseMatrix<T>(r, c);

        std::cout << " Fill: 1. Manual  2. Random (sparse)\n";
        ...
        if (fill_choice == 1) {
            ...
            double density = 0.1;
            double d = read_double(" Density [0.0-1.0] (default 0.1, Enter 0 to skip): ");
            if (d > 0.0 && d <= 1.0) density = d;
            else std::cout << " Using default 0.1\n";
            fill_randomly_sparse(sm, seed, density);
        }
        std::cout << " Created SparseMatrix " << r << "x" << c
                  << " with " << sm->get_data_size() << " non-zero elements.\n";
        return sm;
    }
    ...
    try {
        ...
        case 4: m = new TriangularMatrix<T>(read_int(" Size n: "),
                        TriangularMatrix<T>::Kind::Lower);                    break;
        case 5: m = new TriangularMatrix<T>(read_int(" Size n: "),
                        TriangularMatrix<T>::Kind::Upper);                    break;
...
static void run_soe_work() {
    ...
    if (read_int(" Choice: ") == 2) {
        ...
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n; j++)
                A.set(i, j, read_double(" A[" + std::to_string(i)
                                             + "][" + std::to_string(j) + "] = "));
...
static void experiment_4_2() {
    ...
    std::cout << "\n--- Experiment 4.2: Multiple RHS (n=" << n
              << ", seed=" << seed << ") ---\n";
